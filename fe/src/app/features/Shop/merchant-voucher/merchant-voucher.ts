...
@Component({
  ...
  providers: [DatePipe],
  templateUrl: './merchant-voucher.html',
  styleUrls: ['./merchant-voucher.css']
})
export class MerchantVoucherComponent implements OnInit {
  ...
  loadVouchers() {
    this.isLoading = true;
    this.voucherService.getMerchantVouchers(this.page, this.size, this.filterKeyword)
      .subscribe({
        ...
        error: () => {
          this.toastr.error('Lỗi khi tải danh sách Voucher');
          this.isLoading = false;
        }
      });
  }
  ...
  onSaveVoucher() {
    ...
    if (this.isEditMode && this.editingId) {
      this.voucherService.updateMerchantVoucher(this.editingId, this.formData).subscribe({
        next: () => {
      ...
      this.voucherService.createMerchantVoucher(this.formData).subscribe({
        next: () => {
          this.toastr.success('Tạo Voucher mới thành công!');
          this.showForm = false;
          this.loadVouchers();
        },
        error: (err) => {
